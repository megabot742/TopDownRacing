    ...
    void LapCompleted()
    {
        ...
        else
        {
            ...
            if (!carInputHandler.isAICar)
            {
                ...
                carInputHandler.isAICar = true; // Stop updating Pos, Time, Lap
                if (UIManager.HasInstance)
                {
            ...
            {
                if (UIManager.HasInstance && !UIManager.Instance.isCountingDown && !RaceController.Instance.raceCompleted)//ot is CountDown, and not raceCompleted
                {
