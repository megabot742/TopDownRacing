...
public class CarSFXHandler : MonoBehaviour
{
    ...
    void PasueSound()
    {
        ...
        {
            bool checkPasue = UIEventManager.Instance.isPaused;
            if (checkPasue)
            {
                tireScreenchingAudioSource.Pause();
                speedUpAudioSource.Pause();
                engineAudioSource.Pause();
                carHitAudioSource.Pause();
                carJumpAudioSource.Pause();
                carLandingAudioSource.Pause();
            }
            else
            {
                tireScreenchingAudioSource.UnPause();
                speedUpAudioSource.UnPause();
                engineAudioSource.UnPause();
                carHitAudioSource.UnPause();
                carJumpAudioSource.UnPause();
                carLandingAudioSource.UnPause();
            }
        }
    }
